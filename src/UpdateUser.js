...
const UpdateUser = props => {
    ...
    const updateMission = async (newUsername, newPassword, newFirstName, newLastName, newAttributes) => {
        const requestOptions = {
            method: 'post',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: JSON.stringify({ "OldUserName": userName, "NewUserName": newUsername, "firstname": newFirstName, "lastname": newLastName, "attributes": newAttributes, "password": newPassword })
        };
        await fetch("http://localhost:8080/ReVo_webtest/UpdateUser", requestOptions)
    ...
    return (
        <div className="UserUpdate">
            ...
            <div className="ChangeMission">
                ...
                <button onClick={() => updateMission(newUsername.current.value, newPassword.current.value, newFirstName.current.value, newLastName.current.value, newAttributes.current.value)}>Update User</button>
            </div>
        </div>
    );
}
